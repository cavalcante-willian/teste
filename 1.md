...
```mermaid
flowchart TD
    START([Usuario Inicia Conexao]) --> USB[Conecta Pendrive USB]
    
    USB --> TOKEN{Pendrive<br/>Le Token TOTP<br/>Device ID}
    TOKEN --> BLE{Smartphone BLE<br/>Escaneia UUID<br/>5 segundos}
    
    BLE -->|UUID Detectado| CRED[Envia Credenciais<br/>Token + UUID + GeoIP]
    BLE -->|Nao Detectado| DENY1[Acesso Negado]
    
    CRED --> VALID{VPN Server<br/>Validacao 7 Checks}
    
    VALID -->|Todas OK| AUTH[Autenticado<br/>Chave WG 8h]
    VALID -->|Falha| DENY2[Acesso Negado<br/>Alert Admin]
    
    AUTH --> TUNNEL[Tunel WireGuard<br/>IP 10.99.0.2]
    TUNNEL --> FIREWALL{Palo Alto<br/>Valida Regras}
    
    FIREWALL -->|File Server| ALLOW[Acesso Permitido<br/>User-ID Log]
    FIREWALL -->|AD Server| BLOCK[Bloqueado Sempre]
    
    ALLOW --> FILES[Arquivos Disponiveis<br/>Log Auditoria]
    
    DENY1 --> LOG1[Log: BLE Falhou]
    DENY2 --> LOG2[Log: Validacao Falhou]
    BLOCK --> LOG3[Alert: Tentativa AD]
