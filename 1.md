...
|------------|------|-----------|--------|
| **ISP Fibra 1** | 🔵 Primário | BGP IPv6, AS Próprio, Local Pref 200 | Ativo |
| **ISP Fibra 2** | 🔵 Redundância | BGP IPv6, Standby, Local Pref 100 | Standby |
| **Starlink** | 🛰️ Backup | Emergencial, 100-200 Mbps, NAT | Desconectado |
| **Cloudflare** | ☁️ Proxy | WAF/DDoS, Tunnel permanente | Standby |
| **Backblaze B2** | ☁️ Cloud | Backup offsite, ~20TB criptografado | Ativo |

#### 🔥 Core - Palo Alto PA-3420

**Firewall Central:**
- 25+ VLANs gerenciadas
- BGP Routing (2 peers ativos)
...
- Switch Core: 10.1.1.10-20
- Access Points UniFi: 10.1.1.30-60

**VLANs Domésticas (10.20.X.0/24)**
- **VLAN 10** - Guest WiFi (isolado total, client isolation)
- **VLAN 20** - Home WiFi (dispositivos confiáveis)
- **VLAN 21** - Smartphones (mobile roaming)
- **VLAN 22** - Smart TVs (streaming, QoS alta)
- **VLAN 23** - IoT ⚠️ (Alexa/Google/Lâmpadas, ISOLADO servidores)

**VLANs Corporativas (10.30.1XX.0/24)**
- **VLAN 130** - Workstations (PCs corporativos, proxy obrigatório)
- **VLAN 131** - Impressoras (IPs fixos DHCP)
- **VLAN 133** - Câmeras (NVR + IP cameras, isolado)

**VLANs Servidores (10.30.2XX.0/24)**
- **VLAN 201** - DNS (10.30.201.10, interno + recursivo)
- **VLAN 203** - Web (Nginx/Apache, IPv6 público, Cloudflare only)
- **VLAN 211** - Infra (AD + DHCP/Proxy/Logs + VPN Server)
- **VLAN 215** 🔒 - BACKUP ⚠️ (AIR-GAP 21h/dia, Internet 01:00-04:00 APENAS)
- **VLAN 221** - File + Email (12-16TB ZFS, 2-4TB, ❌ sem acesso externo direto)

#### 🔐 VPN - Acesso Remoto

| Componente | IP | Descrição |
|------------|-----|-----------|
| **VPN Server** | 10.30.211.50 / 2801:...:00D3::50 | WireGuard, Hardware dedicado |
| **Clientes VPN** | 10.99.0.0/24 | Auth 2FA: Pendrive + BLE |

**Controles de Acesso VPN:**
- ✅ Acesso autorizado: File Server (VLAN 221) + Workstations (VLAN 130)
- ❌ BLOQUEADO SEMPRE: AD Server (VLAN 211)
- Logs com User-ID tracking (identifica usuário)

#### 📦 Servidor Infra (10.30.211.30)

- **DHCP Server** - IPv4 + IPv6, 25+ VLANs, WPAD auto-config
- **Squid Proxy** - Cache 50GB, filtro SquidGuard, porta 3128
- **Graylog** - Logs centralizados (firewall/proxy/servers), storage 1-2TB

#### 💾 Servidor Backup (10.30.215.10)

**Camadas de Backup:**
1. **ZFS Pool** - 32 TB RAID-Z2, snapshots 2x/dia, retenção 30d, compressão LZ4
2. **HDDs Externos** - 3x 8TB rotação (diário/semanal/mensal), 🔒 offsite físico
3. **Backup Cloud** - Restic → Backblaze B2, AES-256 client-side, upload 01:00-04:00

**Política Snapshots:**
- Servidores conectam no backup (modelo pull): 08:30-09:30 e 16:30-17:30
- DNS, Web, File/Email, VPN → Backup servidor

#### 🛡️ Regras de Segurança Críticas

| Regra | Descrição | Enforcement |
|-------|-----------|-------------|
| ✅ Backup Air-gap | 21h/dia offline, Internet 01:00-04:00 apenas | Schedule-based |
| ❌ AD Isolamento | ZERO acesso externo, nem via VPN 2FA | Bloqueio total |
| 🔒 File Server | VPN 2FA only, logs User-ID rastreiam | Autenticação |
| 🌐 Cloudflare | Whitelist IPs, acesso direto IP bloqueado | Whitelist |
| 📊 IPv6 | Fixo todos servidores, bloco próprio LACNIC | Nativo |

#### Fluxo de Conectividade

```
Internet (BGP) → Palo Alto PA-3420 → VLANs Segmentadas
                        ↓
        ┌───────────────┼───────────────┐
        ↓               ↓               ↓
   Management        Home/Corp      Servidores
   (Switches/APs)  (Users/IoT)   (DNS/Web/Backup)
        ↓               ↓               ↓
    Monitoring    Proxy/Firewall   Backup 3-2-1
```
...
**1. Workstation solicita acesso à Internet:**
```
Workstation (VLAN 130) → DHCP Server
  └─ Solicita: IP, Gateway, DNS, WPAD
  
 DHCP Server → Workstation
  └─ Responde: 10.30.130.X + configurações

Workstation → Squid Proxy (3128)
  └─ HTTP Request
  
Squid Proxy → Palo Alto
  └─ Filtrado + Cache Check
  
Palo Alto → Internet (BGP)
  └─ Roteado via Fibra 1 (preferencial)
  
Internet → Cloudflare → Web Servers
  └─ Proxy reverso + WAF
```

**2. Acesso VPN ao File Server:**
```
Cliente VPN Remoto (10.99.0.X)
  └─ Autenticação: Pendrive Token + BLE Beacon
  
Cliente → VPN Server (WireGuard)
  └─ Túnel estabelecido (UDP 51820)
  
VPN Server → Palo Alto Firewall
  └─ Valida regras + User-ID logging
  
Palo Alto → File Server (VLAN 221)
  └─ ✅ PERMITIDO: SMB acesso autorizado
  └─ Log: "João Silva acessou File Server via VPN"
```

**3. Tentativa de acesso VPN ao AD (BLOQUEADO):**
```
Cliente VPN → Palo Alto → AD Server (VLAN 211)
  └─ ❌ BLOQUEADO SEMPRE
  └─ Alert enviado ao Admin
  └─ Log: "Tentativa acesso negado - VPN → AD"
```

**4. Fluxo de Backup:**
```
Servidores Produção → Servidor Backup (VLAN 215)
  │
  ├─ File Server: Snapshot 09:00 + 17:00 (validação pré-backup)
  ├─ Web Servers: Backup diário
  ├─ Email Server: Backup diário
  └─ AD Server: Backup diário
  
Servidor Backup → Destinos:
  ├─ HDDs Externos: Cópia física, rotação GFS
  └─ Backblaze B2 Cloud: Upload 01:00-04:00 (restic incremental)
```

**5. Centralização de Logs:**
```
Fontes de Logs → Graylog (Servidor Infra)
  ├─ Palo Alto: Logs firewall (syslog)
  ├─ File Server: Logs acesso (auditoria)
  ├─ Web Servers: Logs HTTP
  └─ Squid Proxy: Logs navegação
  
Graylog: Armazena + indexa + alerta (storage 1-2TB)
```

#### Tabela de Controle de Acesso

| Origem | Destino | Ação | Observação |
|--------|---------|------|-------------|
| Workstation | Internet | ✅ Permitido | Via proxy obrigatório |
| VPN | File Server | ✅ Permitido | User-ID logging |
| VPN | AD Server | ❌ Bloqueado | Sempre negado + alert |
| Servidores | Backup | ✅ Permitido | Janelas: 08:30-09:30, 16:30-17:30 |
| Backup | Cloud | ✅ Permitido | Apenas 01:00-04:00 (air-gap 21h) |
| Internet | Backup | ❌ Bloqueado | Zero inbound |
| Internet | Web | ✅ Permitido | Apenas IPs Cloudflare |
...
#### Fluxo de Controle Visual

**Externo → Interno:**
```
🌐 Internet
  │
  ├─ [Cloudflare IPs] ──────────────────> ✅ Web Servers
  │                          (Whitelist only)
  │
  └─ [Outros IPs] ─────────────────────X ❌ Bloqueado
                            (Força via Cloudflare)

🔐 Cliente VPN (2FA)
  │
  ├─ [File Server] ────────────────────> ✅ Permitido (User-ID log)
  │
  └─ [AD Server] ──────────────────────X ❌ Bloqueado SEMPRE
                                (Alert Admin)
```

**Servidor Backup Air-Gap:**
```
💾 Servidor Backup (VLAN 215)
  │
  ├─ [01:00-04:00] ───────────────────> ✅ Internet (3h janela)
  │                                    Upload Backblaze B2
  │
  └─ [04:00-01:00] ───────────────────X ❌ Bloqueado (21h)
                                   Air-gap ativo

  Inbound:
  🌐 Internet ───────────────────────X ❌ ZERO acesso
                                      (Alert Security)
```
...
**Snapshots ZFS:**
- Horarios: 09:00 (manhã) + 17:00 (tarde)
- Retenção: 30 dias
- Validação pré-backup obrigatória

**CAMADA 2 - FÍSICA (HDDs Externos USB)**

| HDD | Capacidade | Frequência | Tipo | Localização |
|-----|-----------|-----------|------|-------------|
| **HDD 1** | 8 TB | Diário | Incremental | Onsite (conectado Seg-Sex) |
| **HDD 2** | 8 TB | Semanal | Completo | Onsite (rotação 4 semanas) |
| **HDD 3** | 8 TB | Mensal | Completo | 🏦 Offsite físico (cofre/banco) |

**Esquema GFS (Grandfather-Father-Son):**
```
Diário (Son)  ───────> HDD 1 (overwrite semanal)
Semanal (Father) ───> HDD 2 (overwrite mensal)
Mensal (Grandfather) ─> HDD 3 (retenção 12 meses)
```

**CAMADA 3 - CLOUD (Backblaze B2)**

- **Storage:** ~20 TB
- **Criptografia:** AES-256 client-side (restic)
- **Upload:** 01:00-04:00 (janela 3h, air-gap 21h)
- **Tipo:** Incremental + deduplicação automática
- **Retenção:** 30 diários + 12 mensais
- **Verificação:** Semanal (restic check)

#### ✅ Validações Automáticas

**Pré-Backup (antes de cada snapshot):**
1. Ping servidor origem (conectividade)
2. Serviço ativo (SMB, SSH, HTTP)
3. Filesystem clean (sem corrupção)
4. Zero erros I/O (dmesg check)
5. Espaço disco suficiente (< 90%)
...
**Pós-Backup:**
- ✅ Checksum ZFS (valida integridade)
- ✅ Scrub periódico (verifica data integrity)
- ✅ Teste restore mensal (arquivo aleatório)
- ✅ Cloud verify semanal (restic check)
...
🖥️ PRODUÇÃO
  │
  ├─ File Server (12-16 TB)
  ├─ Email Server (2-4 TB)
  ├─ Servidor Infra (1-2 TB)
  ├─ Web Servers (200 GB)
  ├─ VPN Server (100 GB)
  ├─ DNS Server (20 GB)
  └─ AD Server (100 GB)
       │
       ↓ rsync SSH (modelo pull)
       │ Janelas: 08:30-09:30, 16:30-17:30
       │
       v
  💾 CAMADA 1: Servidor Backup (32 TB ZFS)
       │ - Validação pré-backup
       │ - Snapshots 2x/dia
       │ - Retenção 30 dias
       │
       ├────────────────────────────────────┐
       │                                   │
       v                                   v
  💿 CAMADA 2: HDDs Externos        ☁️ CAMADA 3: Backblaze B2
       │ - GFS (Diário/Semanal/Mensal)    │ - AES-256 criptografado
       │ - Offsite físico (HDD 3)          │ - Upload 01:00-04:00
       └─ Rotação manual                  └─ Verificação semanal
```
